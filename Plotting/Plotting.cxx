...
void Plotting::makePlots(){
  ...
  for(; fIter != lIter; ++fIter){
    ...
    fIter->second->SetMarkerSize(0.7);
...
//      Higgs->Fit("gaus" ,"","",120.,130.);
      ...
      if(fIter->first.find("mYY_bin1") != option.npos){
        fit->FixParameter(4,114.4); // this value was obtained from the Gaussian fit
        fit->FixParameter(6,2.51);  // this value was obtained from the Gaussian fit
      }
      if(fIter->first.find("mYY_cat_bin1") != option.npos){
        fit->FixParameter(4,23.02); // this value was obtained from the Gaussian fit
        fit->FixParameter(6,1.85);  // this value was obtained from the Gaussian fit
      }
    ...
    // create the ratio plot  
      TH1F *h_ratio = (TH1F*)Higgs->Clone("h_ratio");
...
//      h_ratio->GetYaxis()->SetTitle("Data / Pred      ");
    ...
    // Calculates the sqrt
    TString originalFormula = fit->GetExpFormula("p");

    // Create a new formula for the square root of 'fit'
    TString sqrtFormula = TString::Format("sqrt(abs(%s))", originalFormula.Data());
      
    // Create a new TF1 object with the square root formula
    TF1 *sqrtFit = new TF1("sqrtFit", sqrtFormula, 105, 160);

      
    if(option.find("HyyAnalysis") != option.npos){
        h_ratio->Divide((TF1*)sqrtFit->Clone());
...
//      delete copy_fit;
    ...
    if(option.find("HyyAnalysis") != option.npos){
        ...
        else{  h_ratio->SetMinimum(-1.);      h_ratio->SetMaximum(2.3);  /*h_ratio->SetMinimum(-125);      h_ratio->SetMaximum(250);*/ }
    }
